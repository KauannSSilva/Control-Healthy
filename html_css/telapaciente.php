...
<html lang="pt-BR">
  <head>
    ...
    <script src="animações.js"></script>
  ...
  <body>
    <div class="container">
      ...
      <main class="form-container">
        ...
        <form id="formInformacoes" autocomplete="on">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- CARREGAR DADOS DO PACIENTE AO ABRIR A PÁGINA (sem alterações) ---
            ...
            .then(data => {
                if (data.status === 'ok' && data.dados) {
                    ...
                    if (data.dados.endereco) {
                        ...
                        preencherFormulario(data.dados.endereco);
                    }
                }
            });

        // --- SALVAR INFORMAÇÕES DO PERFIL (sem alterações) ---
        ...
        // --- ADICIONADO DE VOLTA: LÓGICA DO FORMULÁRIO DE PRESSÃO ---
        formPressao.addEventListener('submit', function(e) {
            ...
            .then(data => {
                mostrarFeedback(data.mensagem, data.status);
                if (data.status === 'ok') {
                    // Limpa apenas os campos de medição, mantendo a data/hora
        ...
        // Função de feedback (sem alterações)
        function mostrarFeedback(mensagem, status) {
            feedbackDiv.textContent = mensagem;
            feedbackDiv.style.color = (status === 'ok') ? 'green' : 'red';
            setTimeout(() => { feedbackDiv.textContent = ''; }, 4000); // Aumentei para 4 segundos
        }
    });
    </script>
  </body>
